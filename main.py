...
def find_obs(ra, dec):
    ...
    for ii in range(len(ts)):
        ...
        g = (x * np.cos(ra * np.pi / 180) * np.sin(dec * np.pi / 180) +
             y * np.sin(ra * np.pi / 180) * np.sin(dec * np.pi / 180) -
             z * np.cos(dec * np.pi / 180))
...
def system_pos(t_obs, ra, dec, ra_off, dec_off, pmra, pmdec, dist, m1, m2, l, period, eccentricity, inclination, Omega,
    ...
    ra_diff_com = ra_off + (x_gaia * np.sin(ra * np.pi / 180) - y_gaia * np.cos(ra * np.pi / 180)) * 1000. / (
                dist * 4.84814e-6) + t_obs * pmra
    dec_diff_com = dec_off + (x_gaia * np.cos(ra * np.pi / 180) * np.sin(dec * np.pi / 180) + y_gaia * np.sin(
        ra * np.pi / 180) * np.sin(dec * np.pi / 180) - z_gaia * np.cos(dec * np.pi / 180)) * 1000. / (
                               dist * 4.84814e-6) + t_obs * pmdec
...
def system_pos_multiple(t_obs, ra, dec, ra_off, dec_off, pmra, pmdec, dist, m1, m2, l, period, eccentricity,
                        inclination, Omega, omega, t_p):
    all_ra_diffs = 0
    all_dec_diffs = 0
    for ii in range(len(m2)):
        ...
        ra_diffs_com = []
        dec_diffs_com = []
        [ra_diff, dec_diff] = radec_diff(t_obs, semimajor, period[ii], q, l[ii], eccentricity[ii], inclination, Omega,
                                         omega[ii], t_p[ii])
...
def gaia_matrix(t_obs, ra, dec):
    ...
    A[len(t_obs):2 * len(t_obs), nVar - 3] = (xG * np.cos(ra * np.pi / 180) * np.sin(dec * np.pi / 180) + yG * np.sin(
        ra * np.pi / 180) * np.sin(dec * np.pi / 180) - zG * np.cos(dec * np.pi / 180)) / 4.84814e-6
...
def gaia_matrix_AL(t_obs, scan_angle, ra, dec):
    ...
    A[:, 2] = (xG * np.sin(ra * np.pi / 180) - yG * np.cos(ra * np.pi / 180)) * np.sin(scan_angle) / 4.84814e-6 + (
                xG * np.cos(ra * np.pi / 180) * np.sin(dec * np.pi / 180) + yG * np.sin(ra * np.pi / 180) * np.sin(
            dec * np.pi / 180) - zG * np.cos(dec * np.pi / 180)) * np.cos(scan_angle) / 4.84814e-6
...
def design_matrix(t_obs, scan_angle, parallax_factor):
    A = np.column_stack([np.sin(scan_angle), np.cos(scan_angle), parallax_factor, t_obs * np.sin(scan_angle),
                         t_obs * np.cos(scan_angle)])
    return A
...
def position3d(rad, true_anomaly, inclination, Omega, omega):
    ...
    x_new = x * (np.cos(Omega * np.pi / 180) * np.cos(omega * np.pi / 180) - np.sin(Omega * np.pi / 180) * np.cos(
        inclination * np.pi / 180) * np.sin(omega * np.pi / 180)) + \
            y * (-np.cos(Omega * np.pi / 180) * np.sin(omega * np.pi / 180) - np.sin(Omega * np.pi / 180) * np.cos(
        inclination * np.pi / 180) * np.cos(omega * np.pi / 180))
    y_new = x * (np.sin(Omega * np.pi / 180) * np.cos(omega * np.pi / 180) + np.cos(Omega * np.pi / 180) * np.cos(
        inclination * np.pi / 180) * np.sin(omega * np.pi / 180)) + \
            y * (-np.sin(Omega * np.pi / 180) * np.sin(omega * np.pi / 180) + np.cos(Omega * np.pi / 180) * np.cos(
        inclination * np.pi / 180) * np.cos(omega * np.pi / 180))
    z_new = x * np.sin(inclination * np.pi / 180) * np.sin(omega * np.pi / 180) + y * np.sin(
        inclination * np.pi / 180) * np.cos(omega * np.pi / 180)
...
def gaia_params_func(x_obs, x_err, gaia_mat):
...
def plx_angle(t, ra, dec, dist):
    ...
    inc = 23.5
    omega = 103
    sin_dec = np.sin(dec * np.pi / 180) - (r / dist) * np.sin(inc * np.pi / 180) * np.sin(
        2 * np.pi * t + omega * np.pi / 180)
    dec_new = np.arcsin(sin_dec)
    dec_diff = dec_new * 180 / np.pi - dec
    tan_ra = (dist * np.cos(dec * np.pi / 180) * np.sin(ra * np.pi / 180) + r * np.cos(inc * np.pi / 180) * np.sin(
        2 * np.pi * t + omega * np.pi / 180)) / \
             (dist * np.cos(dec * np.pi / 180) * np.cos(ra * np.pi / 180) + r * np.cos(
                 2 * np.pi * t + omega * np.pi / 180))
...
def gaia_scan(t, t0, t1):
    ...
    rot_x = np.sin(np.pi / 4) * np.cos(ax_ang) * np.cos(sun_theta) * np.cos(sun_phi) - np.sin(np.pi / 4) * np.sin(
        ax_ang) * np.sin(sun_phi) + np.cos(np.pi / 4) * np.sin(sun_theta) * np.cos(sun_phi)
    rot_y = np.sin(np.pi / 4) * np.cos(ax_ang) * np.cos(sun_theta) * np.sin(sun_phi) + np.sin(np.pi / 4) * np.sin(
        ax_ang) * np.cos(sun_phi) + np.cos(np.pi / 4) * np.sin(sun_theta) * np.sin(sun_phi)
...
def scanning_angles(t):
    [rot_theta, rot_phi, prec_ra, prec_dec, foll_ra, foll_dec] = gaia_scan(t, 34.02 / 365.25,
                                                                           122.4 / (60 * 24 * 365.25))
    [rot_theta0, rot_phi0, prec_ra0, prec_dec0, foll_ra0, foll_dec0] = gaia_scan(t - 1. / (3600 * 24 * 365.25),
                                                                                 34.02 / 365.25,
                                                                                 122.4 / (60 * 24 * 365.25))
...
def thiele_innes(period, parallax, m1, m2, l, inclination, Omega, omega):
    ...
    A = a0 * (np.cos(omega * np.pi / 180) * np.cos(Omega * np.pi / 180) - np.sin(omega * np.pi / 180) * np.sin(
        Omega * np.pi / 180) * np.cos(inclination * np.pi / 180))
    B = a0 * (np.cos(omega * np.pi / 180) * np.sin(Omega * np.pi / 180) + np.sin(omega * np.pi / 180) * np.cos(
        Omega * np.pi / 180) * np.cos(inclination * np.pi / 180))
    F = -a0 * (np.sin(omega * np.pi / 180) * np.cos(Omega * np.pi / 180) + np.cos(omega * np.pi / 180) * np.sin(
        Omega * np.pi / 180) * np.cos(inclination * np.pi / 180))
    G = -a0 * (np.sin(omega * np.pi / 180) * np.sin(Omega * np.pi / 180) - np.cos(omega * np.pi / 180) * np.cos(
        Omega * np.pi / 180) * np.cos(inclination * np.pi / 180))
...
def gaia_diff_col(t_obs, ra, dec, pmra, pmdec, dist):
    [x_gaia, y_gaia, z_gaia] = gaia_position(t_obs)
    ra_diff_com = (x_gaia * np.sin(ra * np.pi / 180) - y_gaia * np.cos(ra * np.pi / 180)) * 1000. / (
                dist * 4.84814e-6) + t_obs * pmra
    dec_diff_com = (x_gaia * np.cos(ra * np.pi / 180) * np.sin(dec * np.pi / 180) + y_gaia * np.sin(
        ra * np.pi / 180) * np.sin(dec * np.pi / 180) - z_gaia * np.cos(dec * np.pi / 180)) * 1000. / (
                               dist * 4.84814e-6) + t_obs * pmdec
...
def best_chain(n_samples, n_chains, A, B, F, G, A_obs, B_obs, F_obs, G_obs):
    ...
    for ii in range(n_chains):
        ...
        diffs.append(np.abs(np.median(A[els]) - A_obs) +
                     np.abs(np.median(B[els]) - B_obs) +
                     np.abs(np.median(F[els]) - F_obs) +
                     np.abs(np.median(G[els]) - G_obs))
    diffs = np.array(diffs)
    return diffs.argmin()


def gaia_query(source_id):
    # Query Gaia DR3 directly using the source_id
    query = f"""
    SELECT 
    ...
    if len(result_table) == 0:
        raise ValueError(f"Source {source_id} not found in Gaia DR3.")

    # Extract parameters from the result table
    params = {}
    keys = [
        'ra', 'dec', 'parallax', 'pmra', 'pmdec',
        'ra_error', 'dec_error', 'parallax_error', 'pmra_error', 'pmdec_error',
    ...
    for key in keys:
        params[key] = result_table[key][0]

    # Find observation times and scanning angles
    ra = params['ra']
    dec = params['dec']
    (t_obs, scan_angle, plx_factor) = find_obs(ra, dec)
...
def find_companion(source_id, nSamps=4000, nChains=1, ast_error='auto', dark=False, circular=False, image_data=None,
                   rv_data=None, init=None):
    (row, t_obs, scan_angle, plx_factor) = gaia_query(source_id)
    # Convert parameters and errors to NumPy arrays so element‐wise operations work:
    gaia_params = np.array(
        [float(row['ra']), float(row['dec']), float(row['parallax']), float(row['pmra']), float(row['pmdec'])])
    gaia_errs = np.array([float(row['ra_error']), float(row['dec_error']), float(row['parallax_error']),
                          float(row['pmra_error']), float(row['pmdec_error'])])
    gaia_corr = [float(row['ra_dec_corr']), float(row['ra_parallax_corr']), float(row['ra_pmra_corr']),
                 float(row['ra_pmdec_corr']), float(row['dec_parallax_corr']), float(row['dec_pmra_corr']),
                 float(row['dec_pmdec_corr']), float(row['parallax_pmra_corr']), float(row['parallax_pmdec_corr']),
                 float(row['pmra_pmdec_corr'])]
    ruwe_obs = float(row['ruwe'])
    g_mag = float(row['phot_g_mean_mag'])

    # Convert t_obs, scan_angle, and plx_factor to NumPy arrays
    t_obs = np.array([float(t) for t in t_obs])
    scan_angle = np.array([float(a) for a in scan_angle])
    plx_factor = np.array([float(p) for p in plx_factor])

    # Debug prints:
    print("DEBUG: gaia_params =", gaia_params)
    print("DEBUG: gaia_errs (type {}):".format(type(gaia_errs)), gaia_errs)
    print("DEBUG: gaia_corr =", gaia_corr)
    print("DEBUG: g_mag =", g_mag)
    print("DEBUG: ruwe_obs =", ruwe_obs)
    print("DEBUG: t_obs =", t_obs)
    print("DEBUG: scan_angle =", scan_angle)
    print("DEBUG: plx_factor =", plx_factor)

    # Choose the proper module based on options:
    if dark:
        if circular:
    ...
    if ast_error == 'auto':
        ...
        err = err if not np.isnan(err) else 0.4
    else:
        err = float(ast_error)

    model = mod.build_model(gaia_params, gaia_errs, gaia_corr, g_mag, ruwe_obs, 0.1,
...
source_id = '6248534171318085376'  # Gaia source id or target name
